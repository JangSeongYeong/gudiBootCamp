...
public class HomeController {
	...
	// index로 요청이 오면 index() 메서드를 실행 해라
	// get 방식으로만 받겠다
	@RequestMapping(value="/index", method = RequestMethod.GET)
	public String index(Model model) { // Model 이라는 녀석으로 jsp 에 데이터를 보낼 수 있다. - MVC 패턴의 Model이랑은 전혀 다른거
		logger.info("/index 요청");
		// name(jsp 에서 el 태그로 부를 때 사용하는 이름), value
		model.addAttribute("msg", "index 페이지에 오셨습니다.");
		return "index"; // 가야할 jsp 이름이기에 String
	}
