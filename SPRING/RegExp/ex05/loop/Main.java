...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("xxx : "+p.matcher("xx").find());
		System.out.println("myxlist : "+p.matcher("myxlist").find());
		System.out.println();
		
		System.out.println("x{n} : x가 n 번 반복");
		p = Pattern.compile("[0-9]{3}"); // 3자리 이상이면 3자리까지 끊어서 가져온다.
		print("1 23 456 7890", p.matcher("1 23 456 7890"));
		
		// 딱 3자리 숫자를 가져오려면?
		p = Pattern.compile("\\b[0-9]{3}\\b");
		print("1 23 456 7890", p.matcher("1 23 456 7890"));
		System.out.println();
		
		System.out.println("x{n,m} : x가 최소 n번, 최대 m번 반복");
		p = Pattern.compile("\\b[0-9]{3,4}\\b");
		print("1 23 456 7890", p.matcher("1 23 456 7890 55555"));
		System.out.println();
		
		// 특정 패턴이 반복되는 것을 가져오고 싶을때
		...
		System.out.println("([0-9])\\1 : x가 1번 이상 반복되면");
		p = Pattern.compile("(\\d)\\1");
		print("두글자 이상 : ", p.matcher("abbcccdddd01223334444"));
		System.out.println();
		
		System.out.println("([\\w])\\1{3,} : 같은 문자가 3번 이상 반복되면");
		p = Pattern.compile("([\\w])\\1{3,}");
		print("세글자 이상 : ", p.matcher("abbcccdddd01223334444"));
		System.out.println();
		...
		for (String phone : phones) {
			System.out.println(phone+" 은 정상적인 번호 인가 ? "+p.matcher(phone).find());
		}
